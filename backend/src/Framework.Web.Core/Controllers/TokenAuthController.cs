...
using Framework.Authentication.TwoFactor.Google;
...
namespace Framework.Web.Controllers
{
    ...
    public class TokenAuthController : FrameworkControllerBase
    {
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            //    if (user != null)
            //    {
            //        if (user.IsActive && await _userManager.IsInRoleAsync(user, "User"))
            //        {
            //            throw new AbpAuthorizationException("Tài khoản không có quyền đăng nhập.");
            //        }
            //    }
            //}
        ...
        private async Task<User> RegisterExternalUserAsync(ExternalAuthUserInfo externalLoginInfo)
        {
            ...
            var user = await _userRegistrationManager.RegisterAsync(
                ...
                externalLoginInfo.Surname + " " + externalLoginInfo.Name,  // vietnamese fullname format
                "",
                "",
                new DateTime(),
                "",
        ...
        private bool ClientIsDesktop()
        {
            var requestUserAgent = Request.Headers["User-Agent"].ToString();
            if (requestUserAgent.Contains("Dart/") && requestUserAgent.Contains("dart:io"))
            {
                return false; 
            }

            return _detectionService.Device.Type == Device.Desktop;
        }
    }
}
