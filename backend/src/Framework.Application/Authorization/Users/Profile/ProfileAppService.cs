...
namespace Framework.Authorization.Users.Profile
{
    ...
    public class ProfileAppService : FrameworkAppServiceBase, IProfileAppService
    {
        ...
        public async Task<bool> CheckOTP(CurrentUserProfileEditDto input)
        {
            var user = await GetCurrentUserAsync();
        ...
        public async Task UpdateProfilePicture(UpdateProfilePictureInput input)
        {
            ...
            if (imageBytes == null)
            {
                throw new UserFriendlyException("Không có hình ảnh có token này: " + input.FileToken);
            }
        ...
        public async Task UpdateAvatarProfile(string url)
        {
            var user = await UserManager.GetUserByIdAsync(AbpSession.GetUserId());
            if(user == null)
            {
        ...
        public async Task<string> GetAvatarProfile()
        {
            var user = await UserManager.GetUserByIdAsync(AbpSession.GetUserId());
